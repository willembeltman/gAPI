...
public class TableListGenerator : BaseGenerator
{
    ...
    public void GenerateCode()
    {
        ...
        var storageFileProps = Dto.Properties
            .Where(p => p.IsStorageFileUrlProperty)
            .ToArray();

        var orderableProps = Dto.Properties
        ...
        <div class=""d-md-none"">
            <div class=""mb-2 d-flex gap-2 align-items-center"">
                <select @onchange=""DataSource.OrderByChanged"" class=""form-select form-select-sm w-auto"">
                    <option value="""" selected=""@string.IsNullOrEmpty(DataSource.OrderByColumn)"">@EmptyText</option>{string.Join("", orderableProps.Select(p => $@"
                    @if (HideColumnNames.Contains(""{p.Name}"") == false)
                    {{
                        <option value=""{p.Name}"" selected=""@{p.Name}Selected"">{p.Name}</option>
                    }}"))}
                </select>
...
@code {{
    ...
    public ListDataSource<{Dto.Name}, {Dto.KeyProperty.TypeSimpleName}>? DataSource {{ get; set; }}

    [Parameter] public string Id {{ get; set; }} = $""{Dto.Name.ToLower()}TableList_{{Guid.NewGuid()}}"";
    ...
    [Parameter] public string NoItemsText {{ get; set; }} = ""No {Dto.Name.ToMultiple()} to display."";
