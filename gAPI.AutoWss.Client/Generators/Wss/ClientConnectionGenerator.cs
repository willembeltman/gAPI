    ...
    private string GenerateArgumentDeclarations(InterfaceMethod method, string requestName, ref string functions, HashSet<string> functionNames)
    {
        ...
        foreach (var (arg, index) in method.Arguments.Select((arg, index) => (arg, index)))
        {
            ...
            result += arg.ParameterType.IsIAsyncEnumerable
                ? $"\n                                var {arg.Name.ToCamelCase()} = RegisterRemoteAsyncEnumerableArgument<{arg.ParameterType.UnderlayingTypes.Single()}>({requestName}.RequestId, {index}, {@method.Interface}_{method}_{index}_Deserializer);"
                    ...
                    {{{string.Join("", hub.Methods.Where(a => a.ResponseType.IsIAsyncEnumerable).Select(method =>
        {
            ...
            return $@"
                        case ""{method}"":
                            {{
                                var ___offset = 0;
                                var ___span = new Span<byte>(___invokeRequest.BinaryData);{GenerateArgumentDeclarations(method, "___invokeRequest", ref functions, functionNames)}
                                foreach (var client in clients)
                                {{
