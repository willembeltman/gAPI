...
{
    public static string CompileRazorToComponent(string fullRazorCode, string namespaceName, string className)
    {
        ...
        string builderCode = builderStructure.Compile();

        // 6. Combineer alles tot class
        var final = new StringBuilder();

        // deduplicate usings and append
        var extraUsings = usings.Distinct().Where(u => !string.IsNullOrWhiteSpace(u)).ToList();
        if (extraUsings.Any())
