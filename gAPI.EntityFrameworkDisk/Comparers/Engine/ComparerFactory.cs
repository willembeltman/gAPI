using gAPI.EntityFrameworkDisk.Comparers.Helpers;
...
internal static class ComparerFactory<TIn, TOut>
{
    ...
    public static ComparerInstance<TIn, TOut> CreateInstance()
    {
        ...
        var @namespace = "gAPI.EntityFrameworkDisk.AutoComparer.GeneratedComparers";
        ...
        var instance = new ComparerInstance<TIn, TOut>(code, isDirtyDelegate);
...
{{
    ...
    if (!gAPI.EntityFrameworkDisk.AutoComparer.Comparer.Compare({sourceExpression}, {targetExpression})) return false;
}}";
    }
