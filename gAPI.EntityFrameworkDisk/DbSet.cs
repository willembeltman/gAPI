using gAPI.EntityFrameworkDisk.Comparers;
...
using gAPI.EntityFrameworkDisk.Navigators;
...
public class DbSet<T> : IDbSet<T>
{
    ...
    public DbSet(DbContext dbContext, DirectoryInfo directory)
    {
        ...
        ComparerInstance = Comparers.Comparer.GetInstance<T, T>();
    ...
    public async Task<T> FirstAsync(Expression<Func<T, bool>> expression, CancellationToken ct)
        => AddToCache(DataSet.First(expression))!;
    public T? FirstOrDefault(Expression<Func<T, bool>> expression)
        => AddToCacheNull(DataSet.FirstOrDefault(expression));
    public async Task<T?> FirstOrDefaultAsync(Expression<Func<T, bool>> expression, CancellationToken ct)
        => AddToCacheNull(DataSet.FirstOrDefault(expression));
